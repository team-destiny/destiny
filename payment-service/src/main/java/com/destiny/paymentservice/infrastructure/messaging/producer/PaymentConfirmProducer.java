...
public class PaymentConfirmProducer {
    ...
    private static final String FAIL_TOPIC = "payment-fail";
    ...
    public void sendSuccess(PaymentSuccessEvent event) {
        try {
            ...
            log.error("Failed to send payment-success: {}", e.getMessage(), e);
            throw new RuntimeException("Kafka 전송 실패 - payment-success", e);
    ...
    public void sendFail(PaymentFailEvent event) {
        try {
            ...
            log.error("Failed to send payment-fail: {}", e.getMessage(), e);
            throw new RuntimeException("Kafka 전송 실패 - payment-fail", e);
        }
    }
}
